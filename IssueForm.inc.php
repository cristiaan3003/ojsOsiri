...
class IssueForm extends Form {
	...
	function validate($issueId = 0) {
		...
		//ccontrola la extension del archivo de imagen y el tamaño maximo permitido de subida	
		...
		$tamao = $_FILES["coverPage"]["size"]; //TAMAÑO de imagen 
                      if ($tamao>102400) {  ///100 kb=102400 bytes
                      $this->addError('coverPage',"El tamaño maximo de imagen es 100kb");
             ...
             //aca controla que sea un css y no otro archivo el que se sube             
	...
	function initData($issueId = null) {
		...
		if (isset($issue)) {
			$this->_data = array(
				...
				'tapaName' => $issue->getTapaName(null), // Localized
				'contraTapaName' => $issue->getContraTapaName(null), // Localized
				'originalFileName' => $issue->getOriginalFileName(null), // Localized
				'originalTapaName' => $issue->getOriginalTapaName(null), // Localized
				'originalContraTapaName' => $issue->getOriginalContraTapaName(null), // Localized
				'coverPageDescription' => $issue->getCoverPageDescription(null), // Localized
				'tapaPageDescription' => $issue->getTapaPageDescription(null), // Localized
				'coverPageAltText' => $issue->getCoverPageAltText(null), // Localized
				'showCoverPage' => $issue->getShowCoverPage(null), // Localized
				'showTapaPage' => $issue->getShowTapaPage(null), // Localized
				'showContraTapaPage' => $issue->getShowContraTapaPage(null), // Localized
	...
	function execute($issueId = 0) {
		...
		$issue->setTapaPageDescription($this->getData('tapaPageDescription'), null); // Localized
		...
		$showContraTapaPage = array_map(create_function('$arrayElement', 'return (int)$arrayElement;'), (array) $this->getData('showContraTapaPage'));
		foreach (array_keys($this->getData('coverPageDescription')) as $locale) {
			if (!array_key_exists($locale, $showContraTapaPage)) {
				$showContraTapaPage[$locale] = 0;
			}
		}
		$issue->setShowContraTapaPage($showContraTapaPage, null); // Localized
		...
		if ($publicFileManager->uploadedFileExists('tapaPage')) {
			$journal = Request::getJournal();
			$originalTapaName = $publicFileManager->getUploadedFileName('tapaPage');
			$type = $publicFileManager->getUploadedFileType('tapaPage');
			$newTapaName = 'tapa_issue_' . $issueId . '_' . $this->getFormLocale() . $publicFileManager->getImageExtension($type);
			//$newTapaName = 'tapa_issue_' . $issueId.'_es_ES';
			$publicFileManager->uploadJournalFile($journal->getId(), 'tapaPage', $newTapaName);
			$issue->setOriginalTapaName($publicFileManager->truncateFileName($originalTapaName, 127), $this->getFormLocale());
			$issue->setTapaName($newTapaName, $this->getFormLocale());

			// Store the image dimensions.
			list($width, $height) = getimagesize($publicFileManager->getJournalFilesPath($journal->getId()) . '/' . $newTapaName);
			$issue->setWidth($width, $this->getFormLocale());
			$issue->setHeight($height, $this->getFormLocale());
		...
		if ($publicFileManager->uploadedFileExists('contraTapaPage')) {
			$journal = Request::getJournal();
			$originalContraTapaName = $publicFileManager->getUploadedFileName('contraTapaPage');
			$type = $publicFileManager->getUploadedFileType('contraTapaPage');
			//$newContraTapaName = 'contratapa_issue_' . $issueId.'_es_ES';
			$newContraTapaName = 'contratapa_issue_' . $issueId . '_' . $this->getFormLocale() . $publicFileManager->getImageExtension($type);
			$publicFileManager->uploadJournalFile($journal->getId(), 'contraTapaPage', $newContraTapaName);
			$issue->setOriginalContraTapaName($publicFileManager->truncateFileName($originalContraTapaName, 127), $this->getFormLocale());
			$issue->setContraTapaName($newContraTapaName, $this->getFormLocale());

			// Store the image dimensions.
			list($width, $height) = getimagesize($publicFileManager->getJournalFilesPath($journal->getId()) . '/' . $newContraTapaName);
			$issue->setWidth($width, $this->getFormLocale());
			$issue->setHeight($height, $this->getFormLocale());
