...
class IssueManagementHandler extends EditorHandler {
	...
	function issueData($args) {
		...
		$templateMgr->assign('helpTopicId', 'publishing.index');
		//$templateMgr->assign('ID_Issue',$issueId);
		$issueForm->display();
	}
	...
	//----------------------------------------------------------------
		/**
	 * Remove tapa from issue
	 */
	function removeIssueTapaPage($args) {
		$issueId = isset($args[0]) ? (int)$args[0] : 0;
		$this->validate($issueId, true);
		...
		$journal =& Request::getJournal();
		$publicFileManager = new PublicFileManager();
		$publicFileManager->removeJournalFile($journal->getId(),$issue->getTapaName($formLocale));
		$issue->setTapaName('', $formLocale);
		$issue->setOriginalTapaName('', $formLocale);
		$issue->setWidth('', $formLocale);
		$issue->setHeight('', $formLocale);
	 ...
	 * Remove tapa from issue
	 */
	function removeIssueContraTapaPage($args) {
		$issueId = isset($args[0]) ? (int)$args[0] : 0;
		$this->validate($issueId, true);
		...
		$journal =& Request::getJournal();
		$publicFileManager = new PublicFileManager();
		$publicFileManager->removeJournalFile($journal->getId(),$issue->getContraTapaName($formLocale));
		$issue->setContraTapaName('', $formLocale);
		$issue->setOriginalContraTapaName('', $formLocale);
		$issue->setWidth('', $formLocale);
		$issue->setHeight('', $formLocale);
	...
	function setupTemplate($level) {
		...
		//parche rapido 
		//para tener un editor master, que solo el pueda publicar  o despublicar los numeros por sobre los demas editores
		//(es es correcto implementarlo asi,debo mejorar la forma de autenticacion)
		//template/editor/issue/issueToc.tpl
		$user = Request::getUser();
		if ($user->getUsername()=='milka'){
		$templateMgr->assign('EditorMaster', 1);
		}
