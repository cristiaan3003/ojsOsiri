...
<input type="hidden" name="tapaName[{$formLocale|escape}]" value="{$tapaName[$formLocale]|escape}" />
<input type="hidden" name="originalTapaName[{$formLocale|escape}]" value="{$originalTapaName[$formLocale]|escape}" />

<input type="hidden" name="contraTapaName[{$formLocale|escape}]" value="{$contraTapaName[$formLocale]|escape}" />
<input type="hidden" name="originalContraTapaName[{$formLocale|escape}]" value="{$originalContraTapaName[$formLocale]|escape}" />
...
<table width="100%" class="data">
	...
	<tr valign="top">
		...
		<td width="80%" class="value"><input type="file" name="coverPage" id="coverPage" 
		class="uploadField" />&nbsp;&nbsp;{translate key="form.saveToUpload"}
		<br />(Formatos permitidos:.png . Tamaño maximo permitido: 100Kb)<br />
		{translate key="editor.issues.uploaded"}:&nbsp;
		
		{if $fileName[$formLocale] }
		<a href="javascript:openWindow('{$publicFilesDir}/{$fileName[$formLocale]|escape:"url"}');" 
		class="file">{$originalFileName[$formLocale]}</a>&nbsp;
		<a href="{url op="removeIssueCoverPage" path=$issueId|to_array:$formLocale}" 
		onclick="return confirm('{translate|escape:"jsparam" key="editor.issues.removeCoverPage"}')">
		{translate key="editor.issues.remove"}	</a>{else}&mdash;
		{/if}
		</td>
	</tr>
		...
		<td width="80%" class="value">
		<input type="checkbox" name="hideCoverPageArchives[{$formLocale|escape}]" 
		id="hideCoverPageArchives" value="1" {if $hideCoverPageArchives[$formLocale]}
		checked="checked"{/if} /> <label for="hideCoverPageArchives">{translate key="editor.issues.hideCoverPageArchives"}</label></td>
	</tr>
	<tr valign="top">
...
<!-- ------------------------------------------------------------------------------------------------- -->
        ...
        <tr valign="top">
              ...
              {if $tapaName[$formLocale] }
		<a href="javascript:openWindow('{$publicFilesDir}/{$tapaName[$formLocale]|escape:"url"}');" 
		class="file">{$originalTapaName[$formLocale]}</a>&nbsp;
		<a href="{url op="removeIssueTapaPage" path=$issueId|to_array:$formLocale}" 
		onclick="return confirm('¿Eliminar imagen de tapa?')">
		{translate key="editor.issues.remove"}	</a>{else}&mdash;
		{/if}
...
<!-- ------------------------------------------------------------------------------------------------- -->
        <tr valign="top">
                <td class="label" colspan="2">
		<input type="checkbox" name="showContraTapaPage[{$formLocale|escape}]" id="showContraTapaPage" value="1" 
		{if $showContraTapaPage[$formLocale]} checked="checked"{/if}
		 /> 
		<label for="showContraTapaPage">Contratapa
		</label>
		</td>
        </tr>

        <tr valign="top">
                <td width="20%" class="label">Imagen de Contratapa</td>
                <td width="80%" class="value">
                        <input type="file" name="contraTapaPage" id="contraTapaPage" class="uploadField" />&nbsp;&nbsp;{translate key="form.saveToUpload"}<br/>
              
              {if $contraTapaName[$formLocale] }
		<a href="javascript:openWindow('{$publicFilesDir}/{$contraTapaName[$formLocale]|escape:"url"}');" 
		class="file">{$originalContraTapaName[$formLocale]}</a>&nbsp;
		<a href="{url op="removeIssueContraTapaPage" path=$issueId|to_array:$formLocale}" 
		onclick="return confirm('¿Eliminar imagen de contratapa?')">
		{translate key="editor.issues.remove"}	</a>{else}&mdash;
		{/if}
                   </td>
        </tr>
